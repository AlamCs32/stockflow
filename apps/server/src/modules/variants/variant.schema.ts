...
export const createVariantSchema = z.object({
  designId: z.coerce.number().int().positive().meta({ description: 'Design the variant belongs to', examples: [1] }),
  colorName: z.string().trim().min(1).max(60).meta({ description: 'Human-readable color name', examples: ['Black'] }),
  colorCode: z
    .string()
    .trim()
    .min(2)
    ...
    .meta({ description: 'Short alphabetic color code used in the SKU (uppercased automatically)', examples: ['BLK'] }),
  size: z
    .string()
    .trim()
    .min(1)
    .max(12)
    .transform((value) => value.toUpperCase())
    .meta({ description: 'Size label used in the SKU (uppercased automatically)', examples: ['XL'] }),
  costPrice: z.coerce.number().positive().max(1_000_000).meta({ description: 'Unit cost price; embedded in the SKU and used for margin calculation', examples: [130] }),
  initialStock: z.coerce.number().int().min(0).default(0).meta({ description: 'Opening stock; logged as an INWARD entry when greater than zero', examples: [50] }),
  samplePhotoUrl: z.string().url().optional().nullable().meta({ examples: ['https://cdn.example.com/kurti-black.jpg'] }),
...
export const upsertPricingSchema = z.object({
  ...
  sellingPrice: z.coerce.number().positive().max(10_000_000).meta({ description: 'Listed price on the channel; margin is computed as this minus variant cost price', examples: [399] }),
});
...
export const adjustStockSchema = z.object({
  quantityChange: z.coerce
    ...
    .meta({ description: 'Signed stock delta. INWARD/RETURN require positive, SALE requires negative.', examples: [-10] }),
  reason: z.enum(StockLogReason).meta({ description: 'Audit reason for the movement' }),
  channel: z.string().trim().min(1).max(60).optional().nullable().meta({ description: 'Optional sales channel the movement is attributed to', examples: ['MEESHO'] }),
});
...
export const channelPricingResponseSchema = z.object({
  ...
  margin: z.number().meta({ description: 'sellingPrice - variant.costPrice, auto-calculated on create and update' }),
  variantId: z.number(),
